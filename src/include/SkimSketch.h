...
public:
	...
	long double Join(Sketch*_other){
		...
		// for(auto x:heavy){
		// 	outStat << x.first.c_str() << " " << x.second << endl;
 		// }
		// outStat << "other " << endl;
		...
		for(auto x:heavy){
			if(other->heavy.count(x.first)){
				// long double p = (long double)x.second*(long double)other->heavy[x.first];
				// outStat << x.second << " " << other->heavy[x.first] << " " << p << endl;
				// cout << p << endl;
				hyre+=(long double)x.second*(long double)other->heavy[x.first];
			}else{
		...
		for (int i = 0; i < d; i++){
			long double k=0;
			for (int j = 0; j < w; j++)
				k+=1ll*(long double)counter[i][j]*(long double)other->counter[i][j];
			res[i]=k;
		}
		sort(res, res + d);
		if (d % 2 == 0)
